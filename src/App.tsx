...
export default function App() {
  const [salaries, setSalaries] = useState<string[]>(Array(12).fill('13000'));
  const [batchSalary, setBatchSalary] = useState<string>('13000');
  
  const [hasSS, setHasSS] = useState(true);
  const [ssBase, setSsBase] = useState<string>('13000');
  const [ssRate, setSsRate] = useState<string>('10.5');
  
  const [hasPF, setHasPF] = useState(true);
  const [pfBase, setPfBase] = useState<string>('13000');
